...
class Bot():
    ...
    def setup_bot(self):
        ...
        async def on_ready():
            ...
            await self.bot.change_presence(game=Game(name="with the wiki", type=1))
    ...
    async def get_stats(self, ctx, args=''):
        ...
        """
        await self.bot.say('Let me find that for you...')
        message_dict = self.format_split_message(ctx.message.content)
        stat = message_dict['args'][0]
        villager_name = message_dict['args'][1]
        ...
        if response:
            ...
            await self.bot.say(f'The {stat} of {villager_name} is: {response}')
            return
        await self.bot.say('Sorry I could not find that for you')
    ...
    async def get_profile_image(self, ctx, args=''):
        ...
        if response:
            await self.bot.say(f'Hey, this seems like a picture of someone I know...\n{response}')
            return
        await self.bot.say('Hm, seems like there are not any pictures of this person I can find')
...
class WikiParser():
    ...
    def request_wiki_info(self, villager_name, stat):
        """
